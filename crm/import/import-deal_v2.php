	...
	if($isExporting) {
		if(intval($dealId) > 0 && $importFile->isDealExist(intval($dealId))) {
			...
			$arFields = array(
				...
				'BEGINDATE' => date('d/m/Y', strtotime($data[OFFERTEDATUM])),
				'CLOSEDATE' => date('d/m/Y', strtotime($data[VERWACHTE_OPDRACHTDATUM])),
				'OPPORTUNITY' => $importFile->cleanMoney($data[RENDEMENT]),
				'PROBABILITY' => $data[SLAGINGSKANS],
		...
		if(intval($dealId) > 0 && $action == "add") {
			...
			$DB->Query($dbUpdateDealID, false, "File: ".__FILE__."<br>Line: ".__LINE__);
			$newID = intval($dealId);
		}
		else if(intval($dealId) > 0 && $action == "update") {
			$newID = $CCrmDeal->Update(intval($dealId), $arFields, true, true, array('REGISTER_SONET_EVENT' => false));
		}

		$importFile->doSaveLog('deal', intval($newID)." - ".print_r($arFields, true));
	}
}
