...
$filePath = $_SERVER['DOCUMENT_ROOT'].'/crm/import/importfile/csv/dealdiff.csv';
...
foreach($arData['rowdata'] as $key => $data) {
	...
	if($isExporting) {
		...
		//dealDiffID = $DB->LastID();
		//$importFile->doSaveLog('deal-diff', intval($dealDiffID)." - ".print_r($mFields, true));
	}
}
