...
if (CSalePaySystemAction::GetParamValue('QUOTE_DE_HEADER_SHOW') == 'Y')
{
	...
	if($sellerPhone)
	{
		$sellerPhone = sprintf("Telefon: %s", $sellerPhone);
	...
	if($sellerEmail)
	{
		$sellerEmail = sprintf("Mail: %s", $sellerEmail);
	...
	if($buyerPayerName)
	{
		$buyerPayerName = sprintf("Ansprechperson: %s", $buyerPayerName);
	...
	if($buyerPhone)
	{
		$buyerPhone = sprintf("Telefon: %s", $buyerPhone);
	...
	if($buyerEmail)
	{
		$buyerEmail = sprintf("Mail: %s", $buyerEmail);
...
if (!empty($arBasketItems))
{
	...
	foreach ($columnList as $column)
	{
		...
		{
			$arCols[$column] = array(
				'NAME' => CSalePdf::prepareToPdf(CSalePaySystemAction::GetParamValue('QUOTE_DE_COLUMN_'.$column.'_TITLE')),
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($arBasket["MEASURE_NAME"] ? $arBasket["MEASURE_NAME"] : 'St.');
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_DE_TOTAL_SHOW') == 'Y')
	{
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(CSalePdf::prepareToPdf("Rabatt:"));
...
for ($n = 1; $n <= $rowsCnt; $n++)
{
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_DE_COLUMN_NAME_SHOW') == 'Y')
	{
...
if (CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false))
{
	$pdf->Cell(
		...
		$pdf->prepareToPdf(
			sprintf(
				'Bitte beachten Sie, dass dieses Angebot bis zum %s gilt.',
