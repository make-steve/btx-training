...
if (CSalePaySystemAction::GetParamValue('QUOTE_LA_HEADER_SHOW') == 'Y')
{
	...
	if($buyerPayerName)
	{
		$buyerPayerName = sprintf("Persona de contacto: %s", $buyerPayerName);
	...
	if($buyerPhone)
	{
		$buyerPhone = sprintf("Teléfono: %s", $buyerPhone);
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($arBasket["MEASURE_NAME"] ? $arBasket["MEASURE_NAME"] : 'pza');
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_LA_TOTAL_SHOW') == 'Y')
	{
		...
		if (!empty($vats))
		{
			...
			foreach ($vats as $vatRate => $vatSum)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(sprintf("Impuesto de ventas (%s%%):", roundEx($vatRate * 100, SALE_VALUE_PRECISION)));
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(CSalePdf::prepareToPdf("Descuento:"));
