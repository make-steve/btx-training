...
function isTaskAllowed2Delete($dealId, $taskId) {

	global $DB;

	$mod_project_task = new mod_project_task(0, 0, false);
	$modTaskField = $mod_project_task->calcFields();
	$modTaskTables = $mod_project_task->calcTables();

	$dbGetTaskData = 'SELECT * from `'.$modTaskTables['ACTUAL'].'` WHERE `DEAL_ID` = ' . intval($dealId) . ' AND `TASK_ID` = ' . intval($taskId);
    $rsGetTaskData = $DB->Query($dbGetTaskData, false, "File: ".__FILE__."<br>Line: ".__LINE__);

    if($arGetTaskData = $rsGetTaskData->fetch()) {

    	$total = 0;
    	foreach($modTaskField as $arField) {

    		if(isset($arGetTaskData[$arField])) {

    			$total += doubleVal($arGetTaskData[$arField]);
    		}
    	}
