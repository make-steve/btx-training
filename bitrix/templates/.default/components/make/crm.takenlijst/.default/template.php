...
foreach($arResult['TASK'] as $sKey =>  $arTask) {
    ...
    $arColumns['UF_TASK_TYPE'] = $arTask['CUSTOM_FIELDS'][CUF_T_TYPE];
    $arColumns['UF_TASK_TYPE'] .= '<input rel="' . $arTask['ID'] . '" type="hidden" name="task['.$arTask['ID'].'][task_type]" value="'.$arTask['CUSTOM_FIELDS'][CUF_T_TYPE].'" />';
    ...
    $arColumns['UF_GEREED'] .= '<input rel="' . $arTask['ID'] . '" type="text" class="percent_done" data-tid="'.$arTask['ID'].'" name="task['.$arTask['ID'].'][percent_done]" placeholder="0" value="'.(($percentDone)?$percentDone:0).'" />';
