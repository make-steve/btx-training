					...
					<div class="crm-lead-header-header-right"><div class="crm-lead-header-right-inner"><?
						if ($arParams['SHOW_STATUS_ACTION'] !== 'N')
						{
							if($entityTypeID === CCrmOwnerType::Deal || $entityTypeID === CCrmOwnerType::Lead || $entityTypeID === CCrmOwnerType::Quote || $entityTypeID === CCrmOwnerType::Invoice):
								...
								if(stripos($arResult['HEAD_PROGRESS_BAR'], "deal closure") !== false) {
									$changeOutput = '<div class="dealpopupheader">Selecteer het resultaat</div><div class="dealpopupbody">Selecteer gewonen indien het voorstel akkoord is door de klant of selecteer verloren als het voorstel afgekeurd is</div>';

									$arResult['HEAD_PROGRESS_BAR'] = str_replace("BX.CrmDealStageManager.messages = {'dialogTitle':'Select the result for the deal closure.','failureTitle':'Deal lost','selectorTitle':'Close deal'}; });", "BX.CrmDealStageManager.messages = {'dialogTitle':'" . $changeOutput . "','failureTitle':'Deal lost','selectorTitle':'Close deal'}; });", $arResult['HEAD_PROGRESS_BAR']);
								}
...
?><script type="text/javascript">
	...
	BX.addCustomEvent("CrmProgressControlAfterSaveSucces", function(progressControl, params)
	{
		...
		if(getParams.TYPE == "DEAL" && getParams.VALUE == "WON")
		{
			...
			var query_data = {
				...
				'dataType': 'json',
				'onsuccess': function(data) {
					if(data.success == true && data.ACTIVE == 'Y')
						popupRedirectProjectCard(data);
				},
			};
	...
	{
		var infoButtons = BX.CrmPopupWindowHelper.prepareButtons(
		[
			{
				type: 'button',
				settings:
				{
					...
					events:
					{
						click : function()
						{
							var popupwindowConfirm = BX.PopupWindowManager.getCurrentPopup();
							popupwindowConfirm.close();
							popupwindowConfirm.destroy();
		...
		var content = '<div style="padding: 16px 12px; max-width: 400px; max-height: 400px; overflow: hidden;">Warning: Client ID Exact is missing!</div>';
	...
	function popupRedirectProjectCard(params)
	{
		...
		var popupWindow = null;
		var content = '<div style="padding: 16px 12px; max-width: 400px; max-height: 400px; overflow: hidden;">Deal is now updated. Would you like to go the project details?</div>';
		popupWindow = BX.PopupWindowManager.create(
	        'lead-dialog-container',
	        BX('menu-popup-item-text'),
	        {
