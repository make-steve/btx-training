...
$taskUrenMoreMulti = $modTask->getSpecificFieldByName('COST_IF_MORE_MULTI');
?>
		...
		<? if ($templateData["SHOW_USER_FIELDS"]): ?>
			<div class="task-detail-properties">
				...
				foreach ($userFields as $userField)
				{
					...
					if($userField['FIELD_NAME'] == $taskUrenMoreMulti) {

						$arFinalValue = array();
						foreach($userField['VALUE'] as $_arValue) {

							$arValues = explode("-", $_arValue);

							if(is_array($arValues) && count($arValues) == 2) {
								
								$dpValue = getUrenDropdownByCode($arValues[0]);
								$textValue = $arValues[1];

								$arFinalValue[] = $dpValue['value'].": ".$textValue;
							}
						}

						$userField['VALUE'] = implode(" ; ", $arFinalValue);
					}
					?>
					<tr>
