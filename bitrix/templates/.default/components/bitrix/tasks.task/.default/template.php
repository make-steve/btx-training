...
<?if(!$hasFatals):?>
	...
	// get crm deal data
	$ownerId = current($arResult['DATA']['TASK']['UF_CRM_TASK']);
	$modTask = new mod_task_data(intval($arParams['ID']));

	$arDefaultValues = array();
	if(strpos($ownerId, "D_") !== false && intval($arParams['ID']) <= 0) {

		$dealId = str_replace("D_", "", $ownerId);
		$modDeal = new mod_deal($dealId);

		$arDeal = $modDeal->getDealInfo();
		$arRevenue = $modDeal->getDealUfValues('REVENUE');
		$revenue = $arRevenue['VALUE'];

		// get responsible person
		$responsibleId = $arDeal['ASSIGNED_BY_ID'];

		if(intval($responsibleId) > 0) {
			$arResult['DATA']['TASK']['SE_RESPONSIBLE'] = array();
			$arResult['DATA']['TASK']['SE_RESPONSIBLE'][] = $modDeal->getResponsible(intval($responsibleId));
		}

		// get productgroup
		$arUfList = $modTask->getUF();
		$arUfdropdown = $modTask->getDropdownUF();

		$productGroup = $arDeal['TYPE_ID'];
		$technology = $modDeal->getDealUfValues('TECHNOLOGY');

		/*$taskData[$arUfList['PRODUCT_GROUP']] = intval($arUfdropdown[$arUfList['PRODUCT_GROUP']][$productGroup]);
		$taskData[$arUfList['TECHNOLOGY']] = intval($arUfdropdown[$arUfList['TECHNOLOGY']][$technology['VALUE']]);*/

		$arDefaultValues = array(
			$arUfList['PRODUCT_GROUP'] => intval($arUfdropdown[$arUfList['PRODUCT_GROUP']][$productGroup]),
			$arUfList['TECHNOLOGY'] => intval($arUfdropdown[$arUfList['TECHNOLOGY']][$technology['VALUE']])
		);

		$defaultKlant = "";
		if(intval($arParams['ID']) <= 0) {
			$arTaskUFFields = $modTask->getTaskUFFields();
			$defaultKlant = $modTask->getDefaultKlant($dealId);
			$arDefaultValues[$arTaskUFFields['REFERENTIE_KLANT']] = $defaultKlant;
		}
	}

	// get caclculation userfield
	$arTaskCalcFiels = $modTask->getCaclFields();
	// get rendement field
	$taskRendement = $modTask->getSpecificFieldByName('YIELD');

	$hiddenKostenUren = $modTask->getSpecificFieldByName('COST_IF_MORE_MULTI');
	$hidenField = array();
	$hidenField[] = array(
		'PARENT' => $modTask->getSpecificFieldByName('COST_IF'),
		'SUB' => array(
			$hiddenKostenUren
		),
		'TEMPLATE' => 'dropdown_text',
		'MORE_PARAMS' => array('DROPDOWN' => getUrenDropdown())
	);
	?>
			...
			<div class="task-options task-options-main">
                            ...
                            <div class="task-options-item-open-inner">
                                ...
                                <span class="task-dashed-link task-dashed-link-add tasks-additional-block-link">
                                    ...
                                    <span class="task-dashed-link-inner" data-bx-id="task-edit-toggler" data-target="accomplice">
                                    	<?=Loc::getMessage('TASKS_TASK_COMPONENT_TEMPLATE_ACCOMPLICES')?></span>
	            ...
	            <div data-bx-id="task-edit-chosen-blocks" class="pinned">
                    ...
                    <div data-bx-id="task-edit-<?=$blockNameJs?>-block" data-block-name="<?=$blockName?>" class="pinable-block task-options-item task-options-item-<?=$blockNameJs?><?=$itemOpenClass?>">
                        ...
                        <div class="task-options-item-open-inner">
                        	<input type="hidden" name="HAS_CUSTOM_FIELDS" value="Y"/>
                            <style type="text/css">
                                .js-id-item-set-item.js-id-item-set-item-305 {
                                    display: none;
                                }
                            </style>
                            <?$APPLICATION->IncludeComponent(
                                ...
                                array(
									...
									'TASKS_CALC_FIELDS' => $arTaskCalcFiels,
									'TASKS_RENDEMENT' => $taskRendement,
									'DEFAULT_VALUES' => $arDefaultValues,
									'HIDDEN_FIELDS' => $hidenField
                                ),
                                null,
                                array("HIDE_ICONS" => "Y")
                            );?>
				...
				<div data-bx-id="task-edit-chosen-blocks" class="pinned">
                    ...
                    <?foreach($arResult['TEMPLATE_DATA']['ADDITIONAL_BLOCKS'] as $blockName):?>
                        ...
                        <?
                        // skip custom fields, since its visible on top
                        if($blockName == 'USER_FIELDS') continue;
                        ...
                        <div data-bx-id="task-edit-<?=$blockNameJs?>-block" data-block-name="<?=$blockName?>" class="pinable-block task-options-item task-options-item-<?=$blockNameJs?><?=$itemOpenClass?>">
                            ...
                            <?elseif($blockName == 'USER_FIELDS' && false):?>
