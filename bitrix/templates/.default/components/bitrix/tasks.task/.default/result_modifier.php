...
{
	// should be the xml ID to the title
	if($arResult['DATA']['TASK']['XML_ID'] != "")
		$sTitle = str_replace("#TASK_ID#", $arResult['DATA']['TASK']['XML_ID'], GetMessage("TASKS_TASK_COMPONENT_TEMPLATE_EDIT_TASK_TITLE"));	
	else
...
if(is_array($arResult['COMPONENT_DATA']['STATE']['BLOCKS']) && is_array($arResult['DATA']['TASK']) && !empty($arResult['DATA']['TASK']))
{
	...
	foreach($arResult['COMPONENT_DATA']['STATE']['BLOCKS'] as $block => $state)
	{
		...
		// marked this as default.
		if($block == 'SE_ORIGINATOR') $chosen = true;
		if($block == 'DATE_PLAN') $chosen = true;
...
// by default the more informaiton is close.
//$hasAdditionalUnchosenFilled = (intval($arResult['DATA']['TASK']['ID']) <= 0) ? false : $hasAdditionalUnchosenFilled;
$hasAdditionalUnchosenFilled = false;
...
// change title
if(!isset($arResult['DATA']['TASK']['ID']) && intval($arResult['DATA']['TASK']['ID']) <= 0) {

	$arResult['DATA']['TASK']['TITLE'] = GetMessage('TASK_DEFAULT_TITLE');
}
...
/*echo '<pre>';
	...
	print_r($arResult);
echo '</pre>';*/
//$arResult['DATA']['TASK']['CUSTOM_FIELDS'] = mod_tasks::getTaskCustomFieldValues($arResult['DATA']['TASK']);