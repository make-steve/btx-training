...
$stageHTML = CCrmViewHelper::RenderDealStageSettings();
$changeOutput = '<div class="popupheader">Selecteer het resultaat</div><div class="popupbody">Selecteer gewonen indien het voorstel akkoord is door de klant of selecteer verloren als het voorstel afgekeurd is</div>';

$stageHTML = str_replace("BX.CrmDealStageManager.messages = {'dialogTitle':'Select the result for the deal closure.','failureTitle':'Deal lost','selectorTitle':'Close deal'}; });", "BX.CrmDealStageManager.messages = {'dialogTitle':'" . $changeOutput . "','failureTitle':'Deal lost','selectorTitle':'Close deal'}; });", $stageHTML);
...
foreach($arResult['DEAL'] as $sKey =>  $arDeal)
{
    ...
    // format the cost value
        ...
        $costFieldIds = array(
                ...
                CUF_D_RENDEMENT, // rendement
                // input[name="OPPORTUNITY"] // rendement
            );

    // get deal's custom fields
    $arDeal['CUSTOM_FIELDS'] = array();
    $selectCf = 'select * from b_uts_crm_deal where VALUE_ID="'.$DB->ForSQL($arDeal['ID']).'" limit 1';
    $rsCf = $DB->Query($selectCf);
    if ($row = $rsCf->Fetch()) {
        $arDeal['CUSTOM_FIELDS'] = $row;
    }

    foreach ($arDeal['CUSTOM_FIELDS'] as $key => $value) {
        if (in_array($key, $costFieldIds)) {
            if (stripos($APPLICATION->GetCurPage(), '/crm/deal/list/')!==false) {
                $cost = strip_tags($value);
                $cost = floatval($cost);
    ...
    /*$opportunity = strip_tags($arDeal['OPPORTUNITY']);
    $opportunity = floatval($opportunity);
    $arDeal['FORMATTED_OPPORTUNITY'] = '€'.number_format($opportunity,2,',','.');*/
    
    // as per IF-63, should use inkomsten instead of rendement
    // format the custom field values
    $amount = $arDeal['CUSTOM_FIELDS'][CUF_D_INKOMSTEN];
    $amount = doubleval($amount);
    $arDeal['FORMATTED_OPPORTUNITY'] = '€'.number_format($amount,2,',','.');
    $arDeal[CUF_D_INKOMSTEN] = '€'.number_format($arDeal[CUF_D_INKOMSTEN],2,',','.');
    // $arDeal[CUF_D_INKOMSTEN] = '€'.number_format($arDeal[CUF_D_INKOMSTEN],2,',','.');
    $arDeal['OPPORTUNITY'] = '€'.number_format($arDeal['OPPORTUNITY'],2,',','.');
    $arDeal['columns']['OPPORTUNITY'] = '€'.number_format($arDeal['columns']['OPPORTUNITY'],2,',','.');
...
$excludeFilters = array(
    'SUM', // amount/currency
    'OPPORTUNITY', // rendement
    CUF_D_INKOMSTEN // income
);

foreach ($arResult['HEADERS'] as $key => $filter) {
    if (in_array($filter['id'], $excludeFilters)) {
        if ('SUM' == $filter['id'])
            unset($arResult['HEADERS'][$key]);

        if ('OPPORTUNITY' == $filter['id'])
            $arResult['HEADERS'][$key]['name'] = 'Rendement';

        if (CUF_D_INKOMSTEN == $filter['id'])
            $arResult['HEADERS'][$key]['name'] = 'Inkomsten';
    }
}
if (isset($_REQUEST['debug'])) {
    echo '<pre>grid_data is ';
print_r($arResult['GRID_DATA']);
echo '</pre>';
}
...
<?endif;?>
<script type="text/javascript">
	BX.addCustomEvent("CrmProcessFailureDialogContentCreated", function(element, wrapper)
	{
		element._onAcceptButtonClick();
