...
$APPLICATION->SetAdditionalCSS('/bitrix/templates/bitrix24/css/auto-complete.css');
...
function setEvents()
{
    ...
    // added autocomplete function for project search
    setTimeout(function() {
        var xhr;
        new autoComplete({
            selector: 'input[id="project_id_input"]',
            minChars: 2,
            source: function(term, suggest){
                try { xhr.abort(); } catch(e){}

                term = term.toLowerCase();
                var geturl = '/crm/deal/action.php';
                xhr = $.getJSON(geturl, { qproject : term,
                            action     : 'select',
                            ajax       : true, }, 
                        function(data){ 
                            // console.log(data);
            ...
            renderItem: function (item, search){
                return '<div class="autocomplete-suggestion" data-langname="'+item.TITLE+'" data-lang="'+item.TITLE+'" data-val="['+item.ID+'] '+item.TITLE+'">['+item.ID+'] '+item.TITLE+'</div>';
            }
        });
    }, 1500);
}
