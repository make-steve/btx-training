		...
		<div id="<?=$arResult['HELPER']->getScopeId()?>" class="tasks">
			...
			<div class="tasks-uf-panel">
				...
				<?if($canUse):?>
					...
					<?php
					$shownFields = mod_task_data::shownFields();
					$formFields = array();
					foreach ($arResult['DATA']['FIELDS'] as $fkey => $fields) {
						if (in_array($fkey, $shownFields)) {
							$formFields[$fkey] = $fields;
						}
					}
					if (!empty($formFields)) {
						$arResult['DATA']['FIELDS'] = $formFields;
					}
					?>
