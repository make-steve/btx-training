...
define('FIELD_CLIENT_NAME', UF_CRM_1511431411);
define('FIELD_TECHNOLOGY', UF_CRM_1493724413);
define('FIELD_PRODUCT_GROUP', 'TYPE_ID');


// get the project details if id is provided
$rsProject = new CDBResult;
$mproject = new MProject;
$projectRow = array();
if (isset($_REQUEST['project'])) {
    $projectid = intval($_REQUEST['project']);
    $rsProject = $mproject->getByID($projectid);
    if (!$projectRow = $rsProject->Fetch()) {
        $projectRow = array();
    }
}
...
// if we are editing an existing deal, get the proper project value
if (empty($projectRow)) {
    $found = false;
    foreach ($arSections as $vSection) {
        foreach ($vSection['FIELDS'] as $vField) {
            if (FIELD_PROJECT_ID == $vField['id']) {
                preg_match('~value="(\d+)"~', $vField['value'], $matches);
                if (!empty($matches)) {
                    $projectid = current($matches);
                    if (stripos($projectid, 'value=')!==false)
                        $projectid = str_ireplace(array('value=', '"'), '', $projectid);
...
.input_currency + input.cost_field,.input_currency + .crm-offer-item-inp {
...
foreach($arSections as &$arSection):
	...
	foreach($arSection['FIELDS'] as &$field):
        ...
        // for custom project ID
        if (FIELD_PROJECT_ID == $field["id"]) {
            if (stripos($APPLICATION->GetCurPage(), '/crm/deal/show/')!==false) {
                if (stripos($field['value'], 'span class="fields')!==false) {
                    $field['value'] = str_ireplace('span class="fields', 'span style="display:none;" class="fields', $field['value']);
                }
            }

            if (stripos($field['value'], '"text"')!==false) {
                $field['value'] = str_ireplace('"text"', '"hidden" id="project_id"', $field['value']);
            }

            if (isset($_REQUEST['project'])) {
                $field['value'] = str_ireplace('value="', 'value="'.intval($_REQUEST['project']), $field['value']);
            }
            if (isset($_REQUEST['project_id'])) {
                $field['value'] = str_ireplace('value="', 'value="'.intval($_REQUEST['project_id']), $field['value']);
            }
            echo $field['value'];
            // echo '<input type="hidden" id="project_id" name="UF_CRM_1505089767" value="" size="20" class="fields integer">';
            continue;
        }

        // client name
        if (FIELD_CLIENT_NAME == $field["id"]) {
            if ($projectRow) {
                $field['value'] = str_ireplace('value="', 'value="'.$DB->ForSQL($projectRow['CLIENT_NAME']), $field['value']);
            }
        }

        // technology
        if (FIELD_TECHNOLOGY == $field["id"]) {
            $field['value'] = str_ireplace('>no</option>', '>- Select -</option>', $field['value']);
            if ($projectRow) {
                $field['value'] = str_ireplace('value="'.$projectRow['TECHNOLOGY'].'"', 'value="'.$projectRow['TECHNOLOGY'].'" selected', $field['value']);
            }
        }

        // product group
        if (FIELD_PRODUCT_GROUP == $field["id"]) {
            
            // $productgroup = array();
            // $productgroup[0] = '- Select -';
            // $productgroup = array_merge($productgroup, $field['items']);
            // $field['items'] = array();
            ...
            if ($projectRow) {
                // $field['value'] = str_ireplace('value="'.$projectRow['PRODUCT_GROUP'].'"', 'value="'.$projectRow['PRODUCT_GROUP'].'" selected', $field['value']);
                $field['value'] = $projectRow['PRODUCT_GROUP'];
            }
            ksort($field['items']);
        }

        // comments
        if ('COMMENTS'==$field["id"]) {
            if ($projectRow) {
                // $field['value'] = $projectRow['COMMENTS'];
            }
        }

        // additional info
        if ('ADDITIONAL_INFO'==$field["id"]) {
            if ($projectRow) {
                // $field['value'] = $projectRow['ADDITIONAL_INFO'];
            }
        }


        $costFieldIds = array(
                CUF_D_KOSTEN_DERDEN, // Kosten derden
                CUF_D_KOSTEN_LIFT, // Kosten LIFT
                CUF_D_KOSTEN_UREN_IF, // uren
                CUF_D_ACQUISITIE, // Aquisitie Kosten
                CUF_D_INKOMSTEN, // inkomsten
                // input[name="OPPORTUNITY"] // rendement
            );

        if (in_array($field["id"], $costFieldIds)) {
            if (stripos($APPLICATION->GetCurPage(), '/crm/deal/show/')!==false && 'custom'==strtolower($field['type'])) {
                $cost = strip_tags($field['value']);
                $cost = floatval($cost);
                $cost = number_format($cost,2,',','.');
                $field['value'] = '<span class="fields double"><span class="fields double">'.$cost.'</span></span>';
            }

            $field['value'] = str_ireplace($field["id"], 'CUSTOM_'.$field["id"], $field['value']);
            $field['value'] = str_ireplace('class="fields integer"', 'style="width: 250px !important; text-indent:40px;" data-target="'.$field['id'].'" class="cost_field fields integer"', $field['value']);

            // if ('UF_CRM_1505978128' == $field["id"]) {
            //     $field['value'] = str_ireplace('data-computed="Y"', ' data-computed="N"', $field['value']);
            // }

        
            $field['value'] = str_ireplace('<input', '<span class="input_currency">€</span><input class="cost_fields_hidden" type="hidden" data-computed="Y" name="'.$field['id'].'"><input', $field['value']);

            if (in_array($field["id"], array(CUF_D_INKOMSTEN, 'UF_CRM_1505962725'))) { // not sure what UF_CRM_1505962725 is
                $field['value'] = str_ireplace('data-computed="Y"', ' data-computed="N"', $field['value']);
            }
        }
		...
		$valEncoded = htmlspecialcharsbx(htmlspecialcharsback(strval($val)));
		...
		elseif($field['type'] === 'bank_details'):
			...
			switch($field['type']):
					...
					case 'custom':
						{
                            ...
                            // for the custom Project ID
                            if (FIELD_PROJECT_ID==$field["id"]) {
                                $val = str_ireplace('type="text"', 'type="hidden" id="project_id"', $val);
                            } elseif (FIELD_PROJECT_NAME==$field["id"]){
                                $origval = $val;
                                $val = str_ireplace('type="text"', 'type="text" id="project_name_disabled" disabled', $val);
                                $val = str_ireplace('name="', 'name="faux_', $val);
                                $hiddenfield = '';
                                if (stripos($origval, 'type="text"')!==false) {
                                    $hiddenfield = str_ireplace('type="text"', 'type="hidden" id="project_name"', $origval);
                                    
                                    // condition:
                                    // name shown: [PROJECT_ID]: [PROJECT_NAME]
                                    // hidden input shown: [PROJECT_NAME]
                                    if (!empty($projectRow)) {

                                        $ptitle = $projectRow['TITLE'];
                                        if (stripos($projectRow['TITLE'], ':')!==false) {
                                            list($tmpId, $ptitle) = explode(':', $projectRow['TITLE']);
                                            $ptitle = trim($ptitle);
                                        }
                                        
                                        // when we are creating a new deal with an existing project
                                        if (stripos($val, 'value=""')!==false) {
                                            $val = str_ireplace('value=""', 'value="'.$projectRow['ID'].': '.$ptitle.'"', $val);
                                            $hiddenfield = str_ireplace('value=""', 'value="'.$ptitle.'"', $hiddenfield);
                                        
                                        } else {
                                            
                                            // when we are editing an existing deal (whose project has ':' in the TITLE)
                                            if (stripos($projectRow['TITLE'], ':')!==false) {
                                                $hiddenfield = str_ireplace('value="'.$projectRow['TITLE'], 'value="'.$ptitle, $hiddenfield);
                                            } else {
                                                
                                                // when creating a new deal with a new project
                                                $val = str_ireplace('value="'.$ptitle, 'value="'.$projectRow['ID'].': '.$ptitle, $val);
                                            }
                                        }
                                    }
                                }
					...
					case 'list':
                        ...
                        <select class="crm-item-table-select" name="<?=$field["id"]?>"<?=$params?>><?
							if(is_array($field["items"])):
                                ...
                                ?><option <?php echo empty($val)?'selected':'';?>>- Select -</option><?php
								foreach($field["items"] as $k=>$v):
									?><option data-test="<?=$k?>" value="<?=htmlspecialcharsbx($k)?>"<?=(in_array($k, $val)? ' selected':'')?>><?=htmlspecialcharsEx($v)?></option><?
								endforeach;
							endif;
							?></select><?
						break;
					...
					case 'crm_locality_search':
						...
						break;
					default:
                        if (stripos($APPLICATION->GetCurPage(), '/crm/deal/edit/')!==false) {
                            $readonly = '';
                            $span = '';
                            $style = '';
                            $placeholder = "";
                            if ('OPPORTUNITY' == $field["id"]) {
                                $readonly = 'readonly';
                                $span = '<span class="input_currency">€</span>';
                                $style = '';
                                $placeholder = "0,00";
                            }
                        }
                        ?><?=$span?><input <?=$readonly?> type="text" class="crm-offer-item-inp <?=$readonly?>" name="<?=$field["id"]?>" placeholder="<?php echo $placeholder; ?>" value="<?=$valEncoded?>"<?=$params?> style="<?=$style?>"><?
				endswitch;
			?></div><?
			if ($advancedInfoHTML !== '')
...
if(!empty($arFieldSets)):
	foreach($arFieldSets as &$arFieldSet):
        ...
        // IF-33; hide the products form on deal edit page
        if ('products' == strtolower($arFieldSet['NAME'])) {
            continue;
        }
