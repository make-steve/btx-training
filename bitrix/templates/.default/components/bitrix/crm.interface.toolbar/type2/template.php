...
$labelText = '';
?>
<?php if (stripos($APPLICATION->GetCurPage(), '/crm/deal/show/')!==false):?>
    <?php
    $currentUri = str_ireplace('/crm/deal/show/', '', $APPLICATION->GetCurUri());
    list($deal, $page) = explode('/', $currentUri);
    $deal = new mod_deal($deal);
    ?>
    <?php if (!$deal->isClosed()):?>
        <a href="javascript:void()" class="main-grid-more-text win_deal">Close Deal</a>
        <script>
        jQuery(document).ready(function() {
            jQuery('.win_deal').on('click', function() {
                $('.crm-stage-won').trigger('click');
                return false;
            });
        });
        </script>
