...
?><div class="webform-buttons" id="<?=htmlspecialcharsbx($toolbarID)?>"><?
...
foreach($arParams["BUTTONS"] as $item):
	...
	{
		$iconClassName = 'crm-menu-bar-btn webform-button webform-button-create';
		if(isset($item['HIGHLIGHT']) && $item['HIGHLIGHT'])
		{
			if($iconClassName !== '')
			{
		...
		$onclick = isset($item['ONCLICK']) ? $item['ONCLICK'] : '';
		?>
			<a class="<?=$iconClassName !== '' ? htmlspecialcharsbx($iconClassName) : ''?>" href="<?=htmlspecialcharsbx($link)?>" title="<?=htmlspecialcharsbx($title)?>" <?=$onclick !== '' ? ' onclick="'.htmlspecialcharsbx($onclick).'; return false;"' : ''?>>
				<span class="webform-button-text"><?=htmlspecialcharsbx($text)?></span>
			</a>
		<?
		if($alignment !== '')
		{
